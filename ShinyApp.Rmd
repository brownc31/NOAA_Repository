...
return(sf) #returns data set for mapping based on user input for threshold value and units
  ...
  output$map <- renderLeaflet({
    ...
    if(as.character(input$var) == "suggested_samples"){ #if the variable chosen is the number of suggested samples:
...
map <-leaflet(sf_data()) %>% 
  ...
  addPolygons(color = ~palette(sf_data()[[input$var]]),
              ...
              fillOpacity = 0.7,
              fillColor = ~palette(sf_data()[[input$var]]),
              popup = ~paste(round(sf_data()[[input$var]], 2))) %>% #then create a simple and less clean map
  addLegend(
    position = "bottomright",
    pal = palette,
    values = sf_data()[[input$var]],
