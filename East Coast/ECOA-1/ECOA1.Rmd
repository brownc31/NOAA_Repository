...
```{r}
ecoa1 <- read_excel("~/NOAA Work/East Coast/ECOA-1/33GG20150619_GLODAP (1).xlsx") %>% 
  slice(-1)
...
ecoa1 <- ecoa1 %>% 
  ...
  mutate(cruise = rep("ECOA1", n())) %>%
  mutate(across(1:8, as.numeric))

write_csv(ecoa1, "~/NOAA Work/East Coast/ECOA-1/ECOA-1-CLEAN.csv")
```
...
```{r}
ggplot() +
  geom_smooth(data = ecoa1, aes(x = oxygen, y = ctdprs), orientation = "y") +
...
```{r}
mean(abs(ecoa1$oxygen - ecoa1$ctdoxy), na.rm = TRUE)
```
